...
Audit-level impact is Guardian's. One audit owns one investigation, including a
change deployed to many devices, and it is off unless `GUARDIAN_ENABLED` is set
on the API and the worker. See [the Guardian design](design/guardian.md) for what
it investigates, what it spends and how it is enabled; nothing below changes with
it on, apart from the per-device AI narrator, which it supersedes.

The timing and collection behavior below describes the per-device monitoring
path, which is unchanged: its verdicts, badges and notifications are the
production ones either way.

## Device monitoring timing

Impact assessments persist the verdict, evidence coverage, relevance plan and
...
for existing indexes and clients. Monitoring passes `legacy_all`, which selects
the evidence every device window collects.
