...
async def test_a_refused_resend_does_not_rotate_the_token(admin_client, use_sender) -> None:
    ...
    accepted = await admin_client.post(
        ...
        json={"token": created["invitation_token"], "password": "a-long-enough-password"},
    )
...
async def test_delivery_detail_never_echoes_a_token_a_server_reflected_back(admin_client, use_sender) -> None:
    ...
    class EchoingSender:
        ...
        async def send(self, *, text: str, **_kwargs: str) -> SendOutcome:
            return SendOutcome("failed", f"Rejected body: ...{text[-80:]}")
...
async def test_failed_and_not_configured_outcomes_still_consume_the_sender_limit(
    fake_users: _FakeUsers,
) -> None:
    ...
    retried without limit - exactly when retrying is most futile.
    """
    ...
    assert first.json()["delivery"] == "not_configured"
    assert second.status_code == 429, "the limit of 1 must already be spent after one not_configured outcome"
