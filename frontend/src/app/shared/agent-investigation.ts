...
export interface AgentCheckpoint {
  ...
  observations: { ref: string; target_handle: string; state: string; sampled_clients: number;
    observed_disconnects: number; gap: string; window: { start: string; end: string } }[];
}
          ...
          <div class="scroll"><table><thead><tr><th>Check reference</th><th>Window</th><th>State</th><th>Sampled clients</th><th>Observed disconnects</th></tr></thead>
            <tbody>@for (evidence of agent.observations; track evidence.ref) {
              <tr><td>{{ evidence.ref }}</td><td>{{ evidence.window.start }}–{{ evidence.window.end }}</td>
                <td>{{ evidence.state }} {{ evidence.gap }}</td><td>{{ evidence.sampled_clients }}</td><td>{{ evidence.observed_disconnects }}</td></tr>
            }</tbody></table></div>
