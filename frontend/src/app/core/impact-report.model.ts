...
export interface ReportDeviceImpact {
  ...
  role: 'affected_switch_port' | 'serving_affected_clients' | 'agent_observed_service';
  service: string;
...
export interface EvidenceDataset {
  ...
  window: { start: string; end: string } | null; captured_at: string; state: string;
...
export interface ImpactReport {
  source?: 'deterministic' | 'mcp_agent';
